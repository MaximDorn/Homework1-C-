...
if (inputLine != null)
{
    ...
    if (array.Length == 5) // Проверка на то, что ввели именно пятизначное число
    {
        ...
        palendromCheck(inputNumber); // Вызов метода, проверяющего пятизначное число на палиндром
    }
    else
    {
...
void palendromCheck(int D) // Метод, проверяющий пятизначное число на палиндром
{
    int D1 = D / 10000;
    int D2 = (D / 1000) % 10;
    int D3 = (D / 100) % 10;
